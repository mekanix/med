...
rsync -P -avcl --delete-after --exclude=.git* --exclude=__pycache__ --exclude=Makefile --exclude=ansible --exclude=build --exclude=cbsd.conf --exclude=local_config.py --exclude=*.db --exclude=*.mk --exclude=requirements.yml --exclude=static --exclude=templates --exclude=tests services/backend/ med.meka.rs:/usr/cbsd/jails-data/medback-data/usr/home/deploy/repos/med/
